#!/usr/bin/env python3
"""Validate the generated Anachronism DragnCards plugin."""
...
def main() -> int:
    ...
    arena_cells = [gid for gid in groups if gid.startswith("sharedArena")]
    if len(arena_cells) != 16:
        errors.append(f"Expected 16 arena cells, found {len(arena_cells)}")
    ...
    print(f"Groups: {len(groups)} (arena cells: {len(arena_cells)})")
    print(f"Layouts: {list(game_def.get('layouts', {}))}")
    print(f"Pre-built decks: {list(game_def.get('preBuiltDecks', {}))}")
    if errors:
        print(f"\n{len(errors)} error(s):")
