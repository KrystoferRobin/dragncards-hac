#!/usr/bin/env python3
"""Validate the generated Wyvern DragnCards plugin."""
...
def parse_tsv(text: str) -> list[list[str]]:
    """Match DragnCards stringTo2DArray (tab-split, newline only on last column)."""
    tokens = text.split("\t")
...
def main() -> int:
    ...
    for layout_id, layout in game_def.get("layouts", {}).items():
        ...
        for option in game_def.get("playerCountMenu", []):
            if option.get("layoutId") == layout_id:
                break
        else:
            # layout may still be valid even if unused
            pass
